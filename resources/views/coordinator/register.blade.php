                    ...
                    <div class="col-md-4">
                        ...
                        <select name="state_id" id="state_id" class="form-select {{ $err('state_id') ? 'is-invalid' : '' }}" required>
                            ...
                            @foreach($states as $state)
                                @php
                                    $code = strtoupper((string) ($state->code ?? ''));
                                    $hours = $state->tc_training_hours;
                                    $nameHi = match ($code) {
                                        'HP', 'HIMACHAL' => 'हिमाचल प्रदेश',
                                        'HR', 'HARYANA' => 'हरियाणा',
                                        default => $state->name,
                                    };
                                @endphp
                                <option value="{{ $state->id }}"
                                    data-name-en="{{ $state->name }}"
                                    data-name-hi="{{ $nameHi }}"
                                    data-hours="{{ $hours !== null && $hours !== '' ? $hours : '' }}"
                                    {{ (string)$v('state_id') === (string)$state->id ? 'selected' : '' }}>
                                    {{ $state->name }}
                                </option>
                            @endforeach
                        </select>
...
<div class="tc-modal-backdrop" id="tc_modal">
    <div class="tc-modal">
        ...
        <div class="tc-modal-body" id="tc_modal_body">
            ...
            <p>मैं अपने ज्ञान, अनुभव एवं कौशल के अनुसार सर्वोत्तम गुणवत्ता का प्रशिक्षण प्रदान करने का संकल्प लेता/लेती हूँ तथा समग्र शिक्षा, <span class="tc-state-hi">—</span> एवं Sane Overseas Private Limited द्वारा निर्धारित सभी नियमों, शर्तों, दिशा-निर्देशों एवं SOP का पूर्णतः पालन करूँगा/करूँगी।</p>
            ...
            <p>मैं यह सुनिश्चित करूँगा/करूँगी कि समग्र शिक्षा, <span class="tc-state-hi">—</span> के अंतर्गत रानी लक्ष्मीबाई आत्म रक्षा प्रशिक्षण (Self-Defence Training) प्रत्येक आवंटित विद्यालय में सफलतापूर्वक संचालित किया जाएगा तथा प्रत्येक विद्यालय में निर्धारित न्यूनतम <span class="tc-hours">—</span> घंटे का प्रशिक्षण पूर्ण कराया जाएगा।</p>
...
<script>
    ...
    function updateTcForSelectedState() {
        const $opt = $('#state_id option:selected');
        const stateId = $opt.val();
        const nameEn = $opt.data('name-en') || '—';
        const nameHi = $opt.data('name-hi') || nameEn;
        let hours = $opt.attr('data-hours');
        if (hours === undefined || hours === null || hours === '') {
            hours = '—';
        }
        $('.tc-state-en').text(nameEn);
        $('.tc-state-hi').text(nameHi);
        $('.tc-hours').text(hours);
        ...
        $.get("{{ url('/trainer-register/state-tc') }}/" + stateId)
            .done(function (res) {
                const en = res.name || nameEn;
                $('.tc-state-en').text(en);
                $('.tc-state-hi').text($opt.data('name-hi') || en);
                let h = res.training_hours;
                if (h === null || h === undefined || h === '') {
                    h = '—';
                } else if (Number(h) === Math.floor(Number(h))) {
                    h = Math.floor(Number(h));
                }
                $('.tc-hours').text(h);
                $opt.attr('data-hours', h === '—' ? '' : h);
            });
    }
